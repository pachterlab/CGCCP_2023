...
def compare_setups(adata, setups, results_dict, hyperparameters, train_index = train_index, test_index = test_index):
  ...
  for setup in setups:
    print(setup, 'with non-linear decoder')
    ...
    df_history = {'reconstruction_error_train_set' : [model.history['reconstruction_loss_train']],
                  'reconstruction_error_test_set': [model.history['reconstruction_loss_validation']]}
    ...
    results_dict[setup]['memory_used'] = torch.cuda.memory_allocated()
    # save model for future testing
    
    print('save path',f'../../results/{method}_model_{name}_linear')
    if 'Bursty' in method:
        model.save(f'../../results/{method}_{name}_MODEL',overwrite=True)
        print('model saved')
...
results_file = open(f"../results/{name}_results_dict_092523.pickle", "wb")
